...
protected:
    // Координаты обновленные
    double x;
    double y;
    double z;
    // Некая величина, в попугаях
    double smth;
    // параметры окружности окружности
    double rad;
    double phi;
    //скорости 
    double vx;
...
public:
    ...
    CalcNode() : x(0.0), y(0.0), z(0.0), smth(0.0), rad(0.0), phi(0.0)
    {
    }

    // Конструктор с указанием всех параметров
    CalcNode(double x, double y, double z, double smth, double rad, double phi) 
        :x(x), y(y), z(z), smth(smth), rad(rad), phi(phi)
    {
    }

    // Метод отвечает за перемещение точки
    // Движемся время tau из текущего положения с текущей скоростью
    void move(double time, double tau) {
        vx = rad * cos(time / 180 * M_PI + phi);
...
public:
    ...
    CalcMesh(const std::vector<double>& nodesCoords, const std::vector<std::size_t>& tetrsPoints) {
        ...
        for(unsigned int i = 0; i < nodesCoords.size() / 3; i++) {
            ...
            double rad = sqrt(nodesCoords[i*3] * nodesCoords[i*3] + nodesCoords[i*3 + 1] * nodesCoords[i*3 + 1]);
            double phi;
            if((pointX < 0 && pointY > 0) || (pointX < 0 && pointY < 0)){
                phi = atan(pointY / pointX) + M_PI;
            }else{
    ...
    void snapshot(unsigned int snap_number) {
        ...
        string fileName = "output/some_physics/some_physics-step-" + std::to_string(snap_number) + ".vtu";
...
int main()
{
    ...
    for(unsigned int step = 1; step < 360; step++) {
        mesh.doTimeStep(step, tau);
        mesh.snapshot(step);
    }
